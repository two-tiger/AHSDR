...
class Onlinevt(ContinualModel):
    ...
    def get_loss_res18(self, inputs, labels, not_aug_inputs,task_id, with_der=False):
        
        task_labels = torch.full_like(labels, task_id)
        outputs = self.net(inputs)
        loss = self.ce_loss(outputs, labels)
        # if self.args.with_der:
        #     F.mse_loss(outputs, buf_logits)
        ...
        loss_sc = 0
        if not self.buffer.is_empty() and task_id > 0:
            #! use attack or trace
            if self.args.use_attack:
                if with_der:
                    ...
                    buf_inputs, buf_labels, _ = self.buffer.get_data(self.args.minibatch_size, self.transform,\
                                                                self.iter,self.print_freq,task_id)
                # buf_inputs = torch.clamp(buf_inputs + 0.1 * torch.randn_like(buf_inputs), 0, 1)
            elif self.args.use_inf:
                buf_inputs, buf_labels, _ = self.buffer.get_data(self.args.minibatch_size,self.transform,\
                                                            self.iter,self.print_freq,task_id)
            ...
            # cat_outputs = self.net(cat_inputs)
            # loss = self.loss(cat_outputs, cat_labels)
    ...
    def get_loss(self, inputs, labels, not_aug_inputs,task_id):
        ...
        if not self.buffer.is_empty() and task_id > 0:
            ...
            if self.args.use_attack :#!use attack
            ...
            inputs = torch.cat((buf_inputs, inputs))
            labels = torch.cat((buf_labels, labels))
        ...
        if self.iter % self.print_freq == 0:
            ...
            print('total loss: ', loss)
        self.iter += 1
        if not self.args.use_attack: #! use random in original code
            # print('no attack')
            if self.args.rand_balance_cls:
                self.buffer.add_data_balanceclass(examples=not_aug_inputs,
                                labels=labels,
