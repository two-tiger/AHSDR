...
def print_incremental_accuracy(mean_acc_list: np.ndarray, task_number: int,
                        setting: str) -> None:
    """
    Prints the mean accuracy on stderr.
    :param mean_acc: mean accuracy value
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        for i in range(len(self.accs_detail)):
            if i < 1:
                ...
                args['task' + str(i + 1) + '_forgetting'] = forgetting(self.accs_detail[:i+1])
        ...
        create_if_not_exists(args['out_dir'] + "/results/" + self.setting)
        create_if_not_exists(args['out_dir'] + "/results/" + self.setting +
                             "/" + self.dataset)
        create_if_not_exists(args['out_dir'] + "/results/" + self.setting +
        ...
        write_headers = False
        path = args['out_dir'] + "/results/" + self.setting + "/" + self.dataset \
               + "/" + self.model + "/mean_accs_{}task.csv".format(self.n_task)
        ...
        if self.setting == 'class-il':
            create_if_not_exists(args['out_dir'] + "/results/task-il/"
                                 + self.dataset)
            create_if_not_exists(args['out_dir'] + "/results/task-il/"
                                 + self.dataset + "/" + self.model)
            ...
            for i in range(len(self.accs_mask_classes_detail)):
                if i < 1:
                    ...
                    args['task' + str(i + 1) + '_forgetting'] = forgetting(self.accs_mask_classes_detail[:i + 1])
            ...
            write_headers = False
            path = args['out_dir'] + "/results/task-il" + "/" + self.dataset + "/"\
                   + self.model + "/mean_accs_{}task.csv".format(self.n_task)
            if not os.path.exists(path):
                write_headers = True
            with open(path, 'a') as tmp:
