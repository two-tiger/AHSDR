...
from mydatasets.utils.multi_dataloader import CudaDataLoader, MultiEpochsDataLoader
...
class ContinualDataset:
    ...
    @staticmethod
    @abstractmethod
    def get_backbone_cct() -> nn.Module:
        """
        Returns the backbone to be used for to the current dataset.
        """
        pass
...
def store_masked_loaders_minist(train_dataset: datasets, test_dataset: datasets,
    ...
    if setting.args.model == 'our':
        ...
        idx = torch.randint(len(train_dataset.data), (int(len(train_dataset.data)
                                                          / current_task_id * (2 * (batch_size) / (batch_size - 4))),))
    else:
...
def store_masked_loaders_core50(train_dataset: datasets, test_dataset: datasets,
                    setting: ContinualDataset) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets,
    ...
    if cuda_dataloader:
        train_loader = MultiEpochsDataLoader(train_dataset, batch_size=setting.args.batch_size,
                                              shuffle=(train_sampler is None),
                                              num_workers=num_workers,
                                              sampler=train_sampler,
                                              pin_memory=pin_memory)

        test_loader = MultiEpochsDataLoader(test_dataset, batch_size=setting.args.batch_size,
                                             shuffle=False,
                                             num_workers=num_workers,
        ...
        train_loader = DataLoader(train_dataset, batch_size=setting.args.batch_size,
                                  shuffle=(train_sampler is None),
                                  num_workers=num_workers,
                                  sampler=train_sampler,
                                  pin_memory=pin_memory)
        test_loader = DataLoader(test_dataset, batch_size=setting.args.batch_size,
                                 shuffle=False,
                                 num_workers=num_workers,
                                 sampler=test_sampler,
                                 pin_memory=pin_memory)
...
def masked_all_loaders_train(train_dataset: datasets,
                    setting: ContinualDataset) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
...
def get_previous_train_loader(train_dataset: datasets, batch_size: int,
    ...
    train_mask = np.logical_and(np.array(train_dataset.targets) >=
        setting.i - setting.N_CLASSES_PER_TASK, np.array(train_dataset.targets) < setting.i)
...
# all data !!!!!!!
def get_all_gan_loader(train_dataset: datasets, batch_size: int, setting: ContinualDataset):
